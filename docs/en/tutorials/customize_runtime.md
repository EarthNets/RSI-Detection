...
You need to create a new directory named `rsidet/core/optimizer`.
And then implement the new optimizer in a file, e.g., in `rsidet/core/optimizer/my_optimizer.py`:
...
- Modify `rsidet/core/optimizer/__init__.py` to import it.

  The newly defined module should be imported in `rsidet/core/optimizer/__init__.py` so that the registry will
...
```python
custom_imports = dict(imports=['rsidet.core.optimizer.my_optimizer'], allow_failed_imports=False)
```

The module `rsidet.core.optimizer.my_optimizer` will be imported at the beginning of the program and the class `MyOptimizer` is then automatically registered.
Note that only the package containing the class `MyOptimizer` should be imported.
`rsidet.core.optimizer.my_optimizer.MyOptimizer` **cannot** be imported directly.
...
- __Use gradient clip to stabilize training__:
  ...
  If your config inherits the base config which already sets the `optimizer_config`, you might need `_delete_=True` to override the unnecessary settings. See the [config documentation](https://rsidetection.readthedocs.io/en/latest/tutorials/config.html) for more details.
...
There are some occasions when the users might need to implement a new hook. MMDetection supports customized hooks in training (#3395) since v2.3.0. Thus the users could implement a hook directly in rsidet or their rsidet-based codebases and use the hook by only modifying the config in training.
Before v2.3.0, the users need to modify the code to get the hook registered before training starts.
Here we give an example of creating a new hook in rsidet and using it in training.
...
Then we need to make `MyHook` imported. Assuming the file is in `rsidet/core/utils/my_hook.py` there are two ways to do that:

- Modify `rsidet/core/utils/__init__.py` to import it.

  The newly defined module should be imported in `rsidet/core/utils/__init__.py` so that the registry will
  find the new module and add it:
...
```python
custom_imports = dict(imports=['rsidet.core.utils.my_hook'], allow_failed_imports=False)
```
...
We implement a customized hook named  [NumClassCheckHook](https://github.com/open-mmlab/rsidetection/blob/master/rsidet/datasets/utils.py) to check whether the `num_classes` in head matches the length of `CLASSES` in `dataset`.

We set it in [default_runtime.py](https://github.com/open-mmlab/rsidetection/blob/master/configs/_base_/default_runtime.py).
...
The config of `evaluation` will be used to initialize the [`EvalHook`](https://github.com/open-mmlab/rsidetection/blob/7a404a2c000620d52156774a5025070d9e00d918/rsidet/core/evaluation/eval_hooks.py#L8).
