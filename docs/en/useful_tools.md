...
[here](#convert-rsidetection-model-to-onnx-experimental).
...
If you need a lightweight GUI for visualizing the detection results, you can refer [DetVisGUI project](https://github.com/Chien-Hung/DetVisGUI/tree/rsidetection).
...
Assume that you have got [Mask R-CNN checkpoint file](https://download.openmmlab.com/rsidetection/v2.0/mask_rcnn/mask_rcnn_r50_fpn_1x_coco/mask_rcnn_r50_fpn_1x_coco_20200205-d4b0c5d6.pth) in the path 'checkpoint'. For other checkpoints, please refer to our [model zoo](./model_zoo.md). You can use the following command to get the results bbox and segmentation json file.
...
```shell
python tools/deployment/rsidet2torchserve.py ${CONFIG_FILE} ${CHECKPOINT_FILE} \
...
### 2. Build `rsidet-serve` docker image

```shell
docker build -t rsidet-serve:latest docker/serve/
```
...
`tools/model_converters/regnet2rsidet.py` convert keys in pycls pretrained RegNet models to
MMDetection style.

```shell
python tools/model_converters/regnet2rsidet.py ${SRC} ${DST} [-h]
```
