...
def convert(in_file, out_file, num_classes):
    ...
    There can be some breaking changes during the development of rsidetection,
    ...
    is_two_stage, is_ssd, is_retina, reg_cls_agnostic = parse_config(
        '#' + meta_info['config'])
    if meta_info['rsidet_version'] <= '0.5.3' and is_retina:
        upgrade_retina = True
    else:
    ...
    if meta_info['rsidet_version'] < '2.5.0':
        upgrade_rpn = True
    else:
        upgrade_rpn = False
